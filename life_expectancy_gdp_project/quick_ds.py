...
class Speedy_Data_Science():
    ...
    def scatter(self, df, column_1: str, column_2: str, xlabel: str, ylabel: str):
        ...
        # Adding code to check if a LOWESS is needed
        """var = input("Is a LOWESS needed? 0 for no, 1 for yes")
        x_in = input("Enter desired x-value")
        y_in = input("Enter desired y-value")
        line_kws_input = {'color': 'black'}
        if var == 1:
            sns.lmplot(x = x_in, y = y_in, line_kws=line_kws_input, lowess= True)
            plt.show()
            plt.close()         
        else:
            print("You chose no, exiting program.")"""
        
    # Good for quantitative data observations, not so much categorical
    def scatter_triple(self, df, column_1: str, column_2: str, column_3: str):
        sns.scatterplot(x = column_1, y = column_2, hue = column_3, palette='bright', data = df)
        plt.savefig('Scatter plot of {}vs{}vs{}.png'.format(column_1, column_2, column_3))
        plt.close()
        
    def scatter_3d_triple(self, df, column_1: str, column_2: str, column_3: str, color_desc):
        df = px.data.iris()
        figure = px.scatter_3d(df, x = column_1, y = column_2, z = column_3, color = color_desc)
        figure.show()
        
    def histogram_single(self, df, column_1: str, color1: str, label1: str, title: str):
        sns.histplot(df[column_1], color=color1, label=label1, alpha=.5)
        plt.legend()
        plt.title(title)
        plt.show()
        plt.close()
    ...
    def count_plot(self, df, column_name: str):
        sns.countplot(x= df[column_name], data=df, order=df[column_name].value_counts().index)
        plt.savefig('{}.png'.format(df[column_name]))
        plt.close()

    def pie_chart(self, df, column_name: str, title: str):
    ...
    def bar_chart(self, df, column_1: str):
        sns.countplot(df[column_1].value_counts(ascending=True).index) # The value_counts(ascending=True) line makes the chart show bars with values least to greatest 
    ...
    #_____Quantitative vs Categorical_____
    
    def boxplot_vs(self, df, cat_column_name, quant_column_name):
        sns.boxplot(x=cat_column_name, y=quant_column_name, data=df, palette='Accent')
        plt.savefig('{} vs {}.png'.format(cat_column_name, quant_column_name))
        plt.close() 
